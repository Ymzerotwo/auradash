...
import { useArticleCategoryFormDialogState } from "@/lib/hooks/useArticleCategories";
...
export function ArticleCategoryFormDialog({
  ...
  const state = useArticleCategoryFormDialogState({ open, onOpenChange, category });
  const {
    isEditing,
        ...
        >
          ...
          <div className="relative px-6 pt-6 pb-5">
            ...
            <DialogHeader className="gap-1.5 pe-8">
              <DialogTitle className="text-lg font-bold text-foreground tracking-tight">
                {isEditing 
                  ? (dict?.categoryForm?.editCategory || dict?.actions?.editCategory || fDict?.editCategory || "Edit Category")
                  : (dict?.categoryForm?.createCategory || dict?.actions?.createCategory || fDict?.createCategory || "Create Category")
                }
              </DialogTitle>
              <DialogDescription className="text-[13px] text-text-muted leading-relaxed m-0">
                {dict?.categoryForm?.fillDetails || "Enter category details below to save."}
              </DialogDescription>
            </DialogHeader>
          </div>
            ...
            <Tabs defaultValue="general" className="w-full h-full flex flex-col pt-5">
              <TabsList className="flex w-full items-center gap-1 bg-surface-subtle border border-border-default rounded-xl p-1 h-auto mb-2">
                <TabsTrigger value="general" className="flex-1 rounded-lg py-2 text-sm font-semibold data-[state=active]:bg-surface-card data-[state=active]:text-foreground data-[state=active]:shadow-sm">
                  {dict?.categoryForm?.general || sDict?.tabs?.general || "General"}
                </TabsTrigger>
                <TabsTrigger value="seo" className="flex-1 rounded-lg py-2 text-sm font-semibold data-[state=active]:bg-surface-card data-[state=active]:text-foreground data-[state=active]:shadow-sm">
                  {dict?.categoryForm?.seo || sDict?.tabs?.seo || "SEO"}
                </TabsTrigger>
              </TabsList>
                ...
                <form id="category-form" onSubmit={handleSubmit}>
                  ...
                  <TabsContent value="general" className="m-0 flex flex-col gap-5 data-[state=inactive]:hidden">
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor="form-title" className="text-xs font-medium text-text-subtle">
                        {fDict?.title} <span className="text-destructive">*</span>
                      </Label>
                      <Input
                        id="form-title"
                        value={title}
                        onChange={(e) => handleTitleChange(e.target.value)}
                        placeholder={fDict?.titlePlaceholder}
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor="form-slug" className="text-xs font-medium text-text-subtle">
                        {fDict?.slug} <span className="text-destructive">*</span>
                      </Label>
                      <Input
                        id="form-slug"
                        value={slug}
                        onChange={(e) => handleSlugChange(e.target.value)}
                        placeholder={fDict?.slugPlaceholder}
                        dir="ltr"
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor="form-excerpt" className="text-xs font-medium text-text-subtle">
                        {dict?.categoryForm?.excerpt || fDict?.excerpt || "Excerpt"}
                      </Label>
                      <Textarea
                        id="form-excerpt"
                        value={excerpt}
                        onChange={(e) => handleExcerptChange(e.target.value)}
                        placeholder={dict?.categoryForm?.excerptPlaceholder || fDict?.excerptPlaceholder || "Short summary shown in category cards..."}
                    ...
                    <div className="flex flex-col gap-1.5">
                      <Label className="text-xs font-medium text-text-subtle">{fDict?.previewImage}</Label>
                      {previewImageUrl ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
                      <div className="flex flex-col gap-1.5">
                        <Label htmlFor="form-sort-order" className="text-xs font-medium text-text-subtle">{fDict?.sortOrder}</Label>
                        <Input
                          id="form-sort-order"
                          type="number"
                          value={sortOrder}
                                ...
                                <div className={`flex items-center justify-between p-2.5 bg-surface-subtle/50 border ${fieldError ? 'border-destructive' : 'border-border-default'} rounded-xl group transition-all hover:border-primary/30`}>
                                  ...
                                  <div className="flex items-center gap-1">
                                    <Button
                                      ...
                                      className="h-7 w-7 p-0 text-text-muted hover:text-amber-400 hover:bg-amber-500/15 opacity-0 group-hover:opacity-100 transition-all rounded-md"
                                    >
                  ...
                  <TabsContent value="seo" className="m-0 flex flex-col gap-5 data-[state=inactive]:hidden pt-2">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <Input 
                        ...
                        placeholder={sDict?.seo?.metaTitleHint || "The title that appears in search results..."}
                        className="h-10 !rounded-lg text-sm" 
                        error={errors['seo_data.meta_title']} 
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <Textarea 
                        ...
                        placeholder={sDict?.seo?.metaDescHint || "A short description of the category..."}
                        ...
                        error={errors['seo_data.meta_description']} 
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      {ogImage ? (
                        ...
                        >
                          ...
                          <span className="text-xs font-medium text-text-subtle">
                            {t.categories?.actions?.chooseShareImage}
                          </span>
                        </Button>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <Input 
                        ...
                        dir="ltr" 
                        className="h-10 !rounded-lg text-sm text-start" 
                        error={errors['seo_data.canonical_url']} 
                      />
                    </div>
          ...
          <div className="px-6 py-4 border-t border-border-default flex items-center justify-end gap-3">
            <DialogClose render={<Button variant="outline" type="button" className="h-10 px-5" />}>
              {dict?.categoryForm?.cancel || fDict?.cancel || dict?.actions?.cancel || "Cancel"}
            </DialogClose>
            ...
            >
              ...
              {saving 
                ? "Saving..."
                : (isEditing 
                    ? (dict?.categoryForm?.save || dict?.actions?.saveChanges || "Save Changes") 
                    : (dict?.categoryForm?.createCategory || dict?.actions?.createCategory || "Create Category")
                  )
              }
            </Button>
          </div>
