...
import { useArticleForm } from "@/lib/hooks/useArticles";

// ─── Props ────────────────────────────────────────────────────────────────────
interface ArticleFormPageProps {
  /** If provided, the form is in "edit" mode and pre-fills fields */
...
// ─── Component ────────────────────────────────────────────────────────────────
export function ArticleFormPage({ article, categoryId }: ArticleFormPageProps) {
  ...
  } = useArticleForm({ article, categoryId });
  ...
  // ─── Render ─────────────────────────────────────────────────────────────────
  return (
    <DashboardLayout pageTitle={isEditing ? af.editTitle : af.createTitle}>
      <div className="flex flex-col gap-6 w-full pb-8 max-w-[1400px] mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground m-0 leading-tight">
                {isEditing ? af.editTitle : af.createTitle}
              </h1>
              <p className="text-sm text-text-muted m-0 mt-0.5">{af.subtitle}</p>
            </div>
          </div>
        ...
        {draftAvailable && (
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
             <div className="flex items-center gap-3">
               <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-600 dark:text-blue-400 shrink-0">
                 <Save size={16} />
               </div>
               <p className="text-sm font-medium text-blue-700 dark:text-blue-300 m-0">
                 {af.draftFound}
               </p>
             </div>
             <div className="flex items-center gap-2 shrink-0">
               <Button type="button" size="sm" variant="ghost" className="text-blue-600 hover:text-blue-700 hover:bg-blue-500/10 dark:text-blue-400 dark:hover:text-blue-300" onClick={handleDiscardDraft}>
                 {af.discardDraft}
               </Button>
               <Button type="button" size="sm" className="bg-blue-600 hover:bg-blue-700 text-white" onClick={handleRestoreDraft}>
                 {af.restoreDraft}
               </Button>
             </div>
          </div>
                  ...
                  <div className="w-full">
                    {previewImageUrl ? (
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() => setIsMediaPickerOpen(true)}
                        className="w-full h-auto aspect-[16/9] border-dashed border-2 border-border-default "
                      >
