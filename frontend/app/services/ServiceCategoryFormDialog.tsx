      ...
      >
        ...
        <div className="relative px-6 pt-6 pb-5">
          ...
          <DialogHeader className="gap-1.5 pe-8">
            <DialogTitle className="text-lg font-bold text-foreground tracking-tight">
              {isEditing ? dict.actions.edit : dict.actions.createCategory}
            </DialogTitle>
            <DialogDescription className="text-[13px] text-text-muted leading-relaxed m-0">
              {formDict.fillDetails}
            </DialogDescription>
          </DialogHeader>
        </div>
          ...
          <Tabs defaultValue="general" className="w-full h-full flex flex-col pt-5">
            ...
            <div className="flex flex-col gap-3 mt-2 pt-5 border-t border-border-default">
              ...
              <TabsContent value="seo" className="m-0 flex flex-col gap-5 data-[state=inactive]:hidden pt-2">
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="cf-seo-title" className="text-xs font-medium text-text-subtle">{settingsDict?.seo?.metaTitle || "Meta Title"}</Label>
                  ...
                  <span className="text-[11px] text-text-muted">{settingsDict?.seo?.metaTitleHint}</span>
                </div>
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="cf-seo-desc" className="text-xs font-medium text-text-subtle">{settingsDict?.seo?.metaDesc || "Meta Description"}</Label>
                  ...
                  <span className="text-[11px] text-text-muted">{settingsDict?.seo?.metaDescHint}</span>
                </div>
                <div className="flex flex-col gap-1.5">
                  ...
                  <span className="text-[11px] text-text-muted">{settingsDict?.seo?.ogImageHint}</span>
                </div>
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="cf-seo-canonical" className="text-xs font-medium text-text-subtle">{settingsDict?.seo?.canonicalUrl || "Canonical URL"}</Label>
                  ...
                  <span className="text-[11px] text-text-muted">{settingsDict?.seo?.canonicalUrlHint}</span>
                </div>
                <div className="flex flex-col gap-1.5 justify-end mt-2">
                  <div onClick={() => setIsIndexable(!isIndexable)} className={`flex items-center justify-between p-2.5 h-10 rounded-xl border transition-all duration-200 cursor-pointer select-none ${isIndexable ? "bg-primary/5 border-primary/40" : "bg-transparent border-border-default hover:border-border-strong"}`}>
                    <span className={`text-sm font-medium transition-colors ${isIndexable ? "text-foreground" : "text-text-subtle"}`}>{settingsDict?.seo?.isIndexable || "Allow Indexing"}</span>
        ...
        <div className="px-6 py-4 border-t border-border-default flex items-center justify-end gap-3">
          ...
          <Button type="button" onClick={handleSubmit} disabled={saving || !title.trim() || !slug.trim()} className="px-5 font-semibold gap-1.5">
            ...
            {saving ? "" : dict.actions.saveChanges}
          </Button>
        </div>
