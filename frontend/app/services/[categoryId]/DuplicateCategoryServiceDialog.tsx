...
/**
 * Duplicate dialog for category-linked services.
 ...
 */
export function DuplicateCategoryServiceDialog({
  open,
  onOpenChange,
  sourceService,
  categoryName,
        ...
        <div className="px-6 py-0 pb-5 max-h-[60vh] overflow-y-auto">
          <Tabs defaultValue="general" className="w-full h-full flex flex-col pt-5">
            ...
            <div className="py-2">
              <TabsContent value="general" className="m-0 flex flex-col gap-5 data-[state=inactive]:hidden">
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="dup-title" className="text-xs font-medium text-text-subtle">{formDict.title}</Label>
                  <Input
                    id="dup-title"
                    value={title}
                    onChange={(e) => {
                ...
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="dup-slug" className="text-xs font-medium text-text-subtle">{formDict.slug} <span className="text-destructive">*</span></Label>
                  <Input
                    id="dup-slug"
                    value={slug}
                    onChange={(e) => {
                ...
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="dup-category" className="text-xs font-medium text-text-subtle">{formDict.categoryId}</Label>
                  <div className="h-10 px-3 bg-surface-subtle border border-border-default rounded-lg flex items-center text-sm text-text-muted select-none">
                    {categoryName || sourceService?.category_id || "..."}
                  </div>
                </div>
            ...
            <TabsContent value="seo" className="m-0 flex flex-col gap-5 data-[state=inactive]:hidden pt-2">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="dup-seo-title" className="text-xs font-medium text-text-subtle">{settingsDict?.seo?.metaTitle || "Meta Title"}</Label>
                <Input id="cf-seo-title" value={metaTitle} onChange={(e) => setMetaTitle(e.target.value)} placeholder={settingsDict?.seo?.metaTitle || "Meta Title"} className="h-10 !rounded-lg text-sm" error={errors['seo_data.meta_title']} />
                <span className="text-[11px] text-text-muted">{settingsDict?.seo?.metaTitleHint || "The title that appears in search results."}</span>
              </div>
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="dup-seo-desc" className="text-xs font-medium text-text-subtle">{settingsDict?.seo?.metaDesc || "Meta Description"}</Label>
                <Textarea id="cf-seo-desc" value={metaDesc} onChange={(e) => setMetaDesc(e.target.value)} placeholder={settingsDict?.seo?.metaDesc || "Meta Description"} className="min-h-[100px] w-full !bg-background border border-input !rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus-visible:border-primary focus-visible:ring-0 focus-visible:shadow-[0_0_15px_rgba(79,70,229,0.25)] transition-all resize-y" error={errors['seo_data.meta_description']} />
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="dup-seo-canonical" className="text-xs font-medium text-text-subtle">{settingsDict?.seo?.canonicalUrl || "Canonical URL"}</Label>
                <Input id="cf-seo-canonical" value={canonicalUrl} onChange={(e) => setCanonicalUrl(e.target.value)} placeholder="https://example.com/category/slug" dir="ltr" className="h-10 !rounded-lg text-sm text-start" error={errors['seo_data.canonical_url']} />
