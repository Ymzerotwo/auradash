...
import { Loader2, Check, Plus, Trash2, Type, AlignLeft, Image as ImageIcon, Video, PlaySquare, Link, Smile, List, Calendar } from "lucide-react";
...
import { useServiceFormDialogState } from "@/lib/hooks/useServices";


export function CategoryServiceFormDialog({
  open,
  onOpenChange,
  service,
  categoryName,
  ...
  const { categoryId: urlCategoryId } = useParams() as { categoryId: string };
  const { t, locale } = useTranslation();
  const dict = t.services;
  const isRtl = locale === "ar";

  const state = useServiceFormDialogState({ open, onOpenChange, service, initialCategoryId: categoryName || urlCategoryId });
  const {
    isEditing,
    title, setTitle,
    slug, setSlug,
        ...
        <div className="relative px-6 pt-6 pb-5">
          ...
          <DialogHeader className="gap-1.5 pe-8">
            <DialogTitle className="text-lg font-bold text-foreground tracking-tight">
              {isEditing ? dict.actions.edit : wizardDict.createService}
            </DialogTitle>
          ...
          <Tabs defaultValue="general" className="w-full h-full flex flex-col pt-5">
            ...
            <div className="py-2">
              <TabsContent value="general" className="m-0 flex flex-col gap-5 data-[state=inactive]:hidden">
                ...
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="form-category" className="text-xs font-medium text-text-subtle">{formDict.categoryId}</Label>
                ...
                {ogImage ? (
                  ...
                  >
                    ...
                    <span className="text-xs font-medium text-text-subtle">
                      {catDict?.actions?.chooseShareImage}
                    </span>
                  </Button>
                )}
